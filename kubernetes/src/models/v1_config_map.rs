...
pub struct V1ConfigMap {
  ...
  /// Data contains the configuration data. Each key must consist of alphanumeric characters, '-', '_' or '.'. ::serde_json::Values with non-UTF-8 byte sequences must use the BinaryData field. The keys stored in Data must not overlap with the keys in the BinaryData field, this is enforced during validation process.
  #[serde(rename = "data")]
...
impl V1ConfigMap {
  ...
  pub fn with_binary_data(mut self, binary_data: ::std::collections::HashMap<String, String>) -> V1ConfigMap {
    self.binary_data = Some(binary_data);
    self
  }
  ...
  pub fn with_metadata(mut self, metadata: ::models::V1ObjectMeta) -> V1ConfigMap {
    self.metadata = Some(metadata);
    self
  }
