...
pub trait RbacAuthorizationApi {
    fn get_rbac_authorization_api_group(&self, ) -> Box<Future<Item = ::models::IoK8sApimachineryPkgApisMetaV1ApiGroup, Error = Error<serde_json::Value>>>;
}


impl<C: hyper::client::Connect>RbacAuthorizationApi for RbacAuthorizationApiClient<C> {
    fn get_rbac_authorization_api_group(&self, ) -> Box<Future<Item = ::models::IoK8sApimachineryPkgApisMetaV1ApiGroup, Error = Error<serde_json::Value>>> {
        __internal_request::Request::new(hyper::Method::Get, "/apis/rbac.authorization.k8s.io/".to_string())
