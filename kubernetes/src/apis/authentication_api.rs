...
pub trait AuthenticationApi {
    fn get_authentication_api_group(&self, ) -> Box<Future<Item = ::models::IoK8sApimachineryPkgApisMetaV1ApiGroup, Error = Error<serde_json::Value>>>;
}


impl<C: hyper::client::Connect>AuthenticationApi for AuthenticationApiClient<C> {
    fn get_authentication_api_group(&self, ) -> Box<Future<Item = ::models::IoK8sApimachineryPkgApisMetaV1ApiGroup, Error = Error<serde_json::Value>>> {
        __internal_request::Request::new(hyper::Method::Get, "/apis/authentication.k8s.io/".to_string())
